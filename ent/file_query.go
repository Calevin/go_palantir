...
func (fq *FileQuery) QueryTokens() *TokenQuery {
	query := (&TokenClient{config: fq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, file.TokensTable, file.TokensColumn),
		)
...
func (fq *FileQuery) Clone() *FileQuery {
	...
	return &FileQuery{
		...
		withTokens: fq.withTokens.Clone(),
...
func (fq *FileQuery) WithTokens(opts ...func(*TokenQuery)) *FileQuery {
	query := (&TokenClient{config: fq.config}).Query()
...
func (fq *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if query := fq.withTokens; query != nil {
		if err := fq.loadTokens(ctx, query, nodes,
			func(n *File) { n.Edges.Tokens = []*Token{} },
			func(n *File, e *Token) { n.Edges.Tokens = append(n.Edges.Tokens, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FileQuery) loadTokens(ctx context.Context, query *TokenQuery, nodes []*File, init func(*File), assign func(*File, *Token)) error {
	...
	query.Where(predicate.Token(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(file.TokensColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.file_tokens
		if fk == nil {
			return fmt.Errorf(`foreign-key "file_tokens" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "file_tokens" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
